...
#define TMENU_MAX	(sizeof(main_menu)/sizeof(Menu_pack))
...
VshConfigMini config_mini;
VshConfigMini config_mini_backup;

int *main_menu_switch = (void *)config_mini.menu_switch;

//int main_menu_switch[TMENU_MAX];
...
int module_start(int argc, char *argv[])
{
	...
	if (fd >= 0)
	{
		int read_size = sceIoRead( fd , &config_mini , sizeof(VshConfigMini) );
		sceIoClose(fd);

		if( read_size >= 4 && config_mini.magic == MINI_MAGIC )
		{
		...
		{
			scePaf_memset( &config_mini, 0 , sizeof(VshConfigMini));
		}
	}


	scePaf_memcpy( &config_mini_backup, &config_mini, sizeof(VshConfigMini));
...
int TSRThread(SceSize args, void *argp)
{
	...
	if( scePaf_memcmp( &config_mini_backup, &config_mini, sizeof(VshConfigMini) ) != 0)
	{
		int fd= sceIoOpen("flashfat1:/vsh.me", PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777);
		if (fd >= 0)
		{
			config_mini.magic = MINI_MAGIC;
			sceIoWrite( fd , &config_mini , sizeof(VshConfigMini) );
			sceIoClose(fd);
		}
	}
...
//	int main_menu_switch_backup[TMENU_MAX];
//	scePaf_memcpy( main_menu_switch_backup, main_menu_switch, sizeof(main_menu_switch) );
//	scePaf_memset( main_menu_switch, 0, sizeof(main_menu_switch) );
